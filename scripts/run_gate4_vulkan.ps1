...
Write-Host " TRIWORLD V4 GATE 4 — VULKAN RUNTIME TEST & LOG AUDIT" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
...
$BeamNgExe = $BeamNgExePaths | Where-PathExists | Select-Object -First 1

Write-Host "`nInstructions for Visual Verification:" -ForegroundColor Yellow
...
Write-Host "4. Capture 3 screenshots (Top-Down, Ground Level, Wheel Contact) and save to:" -ForegroundColor Yellow
Write-Host "   $ArtifactsDir" -ForegroundColor Yellow

if (Test-Path $BeamNgLogPath) {
    Write-Host "`nScanning previous BeamNG log for potential errors..." -ForegroundColor Cyan
    $LogContent = Get-Content $BeamNgLogPath -ErrorAction SilentlyContinue
    $Errors = $LogContent | Select-String -Pattern "error|failed|Vulkan|TSStatic|material|texture" -CaseSensitive:$false
    if ($Errors) {
        Write-Host "Found $($Errors.Count) relevant lines in beamng.log:" -ForegroundColor Gray
