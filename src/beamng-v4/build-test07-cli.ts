...
const GENERATED = path.resolve('src/beamng-v4/.generated-build-test07-cli.ts');
...
function main(): void {
  ...
  transformed = replaceExactly(
    ...
    "    title: 'TEST07 — Visible Road Material Diagnostic',",
    'title',
  );
  ...
  try {
    ...
    if (result.status !== 0) throw new Error(`test07 build exited with code ${result.status}.`);
  } finally {
...
try { main(); } catch (error) {
  console.error('FATAL TEST07 BUILD ERROR:', error);
  process.exit(1);
}
