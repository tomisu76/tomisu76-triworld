...
const GENERATED = path.resolve('src/beamng-v4/.generated-build-roadfix02-cli.ts');
...
function main(): void {
  ...
  const debugMarkers: LevelMarker[] = [{
    ...
    'safe 180-degree projected-road spawn',
  );
  ...
  transformed = replaceExactly(
    ...
    "    title: 'ROADFIX02 — Orthophoto-Aligned Terrain Projected Road',",
    'title',
  );
  ...
  levelFiles.materialsJson = JSON.stringify({
    ...
    roadfix02_asphalt: {
      name: 'roadfix02_asphalt',
      mapTo: 'roadfix02_asphalt',
      class: 'Material',
      internalName: 'roadfix02_asphalt',
      persistentId: 'e4873851-e86f-432c-b36d-e56c12550b02',
      version: 1.5,
      Stages: [{
  ...
  // The accepted orthophoto is rotated 180 degrees for BeamNG terrain UVs.
  // Apply the same X+Y transform to the native DecalRoad nodes.
  ...
  const distDir = path.resolve('dist');`,
    'hard terrain and orthophoto-aligned projected DecalRoad',
  );
  ...
  try {
    ...
    if (result.status !== 0) throw new Error(`roadfix02 build exited with code ${result.status}.`);
  } finally {
...
try {
  ...
  console.error('FATAL ROADFIX02 BUILD ERROR:', error);
  process.exit(1);
}
