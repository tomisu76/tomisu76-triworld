...
const GENERATED = path.resolve('src/beamng-v4/.generated-build-test09-cli.ts');
...
function main(): void {
  ...
  const spawnX = WORLD_RUNTIME_SPAN - spawnLogicalX;
  const spawnY = WORLD_RUNTIME_SPAN - spawnLogicalY;
  const spawnZ = sampleTerrainElevation(spawnLogicalX, spawnLogicalY) + 5.0;
  ...
  transformed = replaceExactly(
    ...
    "    title: 'TEST09 — Controlled 180 Degree Runtime Frame',",
    'title',
  );
  ...
  if (!roadObject) throw new Error('test09 expected road_surface_mesh in items.level.json.');
  ...
  const distDir = path.resolve('dist');`,
    'hard terrain and controlled visual road',
  );
  ...
  try {
    ...
    if (result.status !== 0) throw new Error(`test09 build exited with code ${result.status}.`);
  } finally {
...
try { main(); } catch (error) {
  console.error('FATAL TEST09 BUILD ERROR:', error);
  process.exit(1);
}
