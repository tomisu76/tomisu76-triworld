...
const GENERATED = path.resolve('src/beamng-v4/.generated-build-test05-cli.ts');
...
function main(): void {
  ...
  transformed = replaceExactly(
    ...
    "    title: 'TEST05 — Hard Terrain Collision Isolation',",
    'title',
  );
  ...
  try {
    ...
    if (result.status !== 0) throw new Error(`test05 build exited with code ${result.status}.`);
  } finally {
...
try { main(); } catch (error) {
  console.error('FATAL TEST05 BUILD ERROR:', error);
  process.exit(1);
}
