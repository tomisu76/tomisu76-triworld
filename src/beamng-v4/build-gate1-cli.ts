/**
 * CLI Generator for TriWorld V4 Gate 1 — Real-World Terrain (Bánovce nad Bebravou)
 */
...
async function main() {
  console.log('Building TriWorld V4 Gate 1 Real-World Terrain (Bánovce nad Bebravou)...');
  ...
  const levelFiles = generateLevelPackageFiles(artifact, {
    title: 'TriWorld V4 Native Gate 1 (Bánovce Real-World Terrain)',
    description: 'Bánovce nad Bebravou GIS DEM real-world terrain level',
  ...
  const zipPath = path.join(distDir, 'triworld_v4_gate1.zip');
  const manifestPath = path.join(distDir, 'triworld_v4_gate1.manifest.json');
  ...
  console.log('GIS Manifest details:\n', JSON.stringify(gisManifest, null, 2));

  // Copy to BeamNG mods directories
  const targetModsPaths = [
    'C:\\Users\\tomisu\\AppData\\Local\\BeamNG\\BeamNG.drive\\current\\mods\\triworld_v4_gate1.zip',
    'C:\\Users\\tomisu\\AppData\\Local\\BeamNG.drive\\0.36\\mods\\triworld_v4_gate1.zip',
  ];
...
main().catch((err) => {
  console.error('Gate 1 CLI Build Failed:', err);
  process.exit(1);
});
